...
#[allow(clippy::await_holding_lock)]
async fn assert_codex_http_body_error_surfaces_downstream(
    first_body: Vec<u8>,
    ...
    let (body_bytes, body_error) = tokio::time::timeout(Duration::from_secs(2), async {
...
async fn spawn_websocket_exhausted_rate_limit_upstream() -> String {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    ...
    tokio::spawn(async move {
        ...
        {
            let _ = ws.next().await;
            let snapshot = r#"{"type":"codex.rate_limits","rate_limits":{"allowed":false,"limit_reached":true,"primary":{"reset_after_seconds":518773}},"credits":{"has_credits":false,"unlimited":false}}"#;
...
#[allow(clippy::await_holding_lock)]
#[tokio::test]
async fn smoke_codex_http_exhausted_quota_eof_returns_429() {
    let _guard = env_lock();
    clear_all_continuations_for_tests();
    let config = TempDir::new().unwrap();
    write_auth(config.path(), "codex");
    ...
    for stream in [false, true] {
        ...
        let upstream = spawn_http_upstream({
            ...
            move |_body: Value| {
                attempts.fetch_add(1, Ordering::SeqCst);
                "data: {\"type\":\"codex.rate_limits\",\"rate_limits\":{\"allowed\":false,\"limit_reached\":true,\"primary\":{\"reset_after_seconds\":518773}},\"credits\":{\"has_credits\":false,\"unlimited\":false}}\n\n"
                    .as_bytes()
                    .to_vec()
            }
        })
        ...
        assert_eq!(response.headers()[http::header::RETRY_AFTER], "518773");
        let body = axum::body::to_bytes(response.into_body(), usize::MAX)
            .await
            .unwrap();
        let value: Value = serde_json::from_slice(&body).unwrap();
...
#[allow(clippy::await_holding_lock)]
#[tokio::test]
async fn smoke_codex_http_body_error_after_semantic_output_surfaces_downstream() {
    let text = assert_codex_http_body_error_surfaces_downstream(
        concat!(
            "data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_partial\"}}\n\n",
            "data: {\"type\":\"response.output_item.added\",\"output_index\":0,\"item\":{\"type\":\"message\",\"id\":\"msg_partial\"}}\n\n",
...
#[allow(clippy::await_holding_lock)]
#[tokio::test]
async fn smoke_codex_http_body_error_after_closed_text_surfaces_downstream() {
    let text = assert_codex_http_body_error_surfaces_downstream(
        concat!(
...
#[allow(clippy::await_holding_lock)]
#[tokio::test(flavor = "multi_thread")]
async fn smoke_codex_websocket_exhausted_quota_close_returns_429() {
    let _guard = env_lock();
    let config = TempDir::new().unwrap();
    write_auth(config.path(), "codex");
    ...
    for stream in [false, true] {
        clear_codex_websocket_pool_for_tests();
        let upstream = spawn_websocket_exhausted_rate_limit_upstream().await;
        let _config_env = EnvGuard::set("CCP_CONFIG_DIR", config.path());
        let _base_url_env = EnvGuard::set("CCP_CODEX_BASE_URL", &upstream);
        let _transport_env = EnvGuard::set("CCP_CODEX_TRANSPORT", "websocket");
