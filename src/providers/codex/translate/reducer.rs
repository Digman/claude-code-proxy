...
use crate::providers::codex::events::{
    CodexEventFailure, CodexFailureKind, classify_event_failure,
    is_standard_max_output_tokens_incomplete, quota_failure_hint, response_is_incomplete_terminal,
};
...
pub(crate) fn reduce_upstream_bytes_with_policy(
    ...
    for evt in &sse_events {
        ...
        if !allowed_incomplete && let Some(failure) = classify_event_failure(&p) {
            return Err(upstream_error_from_failure(failure));
        }

        if t == "codex.rate_limits" {
            quota_failure = quota_failure_hint(&p);
    ...
    if !_saw_terminal && let Some(failure) = quota_failure {
        return Err(upstream_error_from_failure(failure));
    }
...
fn upstream_error_from_failure(failure: CodexEventFailure) -> UpstreamStreamError {
