...
If a retryable HTTP or WebSocket transport failure interrupts output while a downstream content block is still open, the proxy leaves the stream incomplete so Claude Code can use its built-in non-streaming fallback. A completed tool call is finalized with `tool_use` instead of being replayed. If the text block already closed before the transport failed, the proxy returns an explicit stream error rather than silently accepting truncated output. Leave `CLAUDE_CODE_DISABLE_NONSTREAMING_FALLBACK` unset to allow recovery; setting it to `1` opts out. The fallback sends another model request, so recovered text can differ from the interrupted output. Explicit upstream API errors are still returned directly.
